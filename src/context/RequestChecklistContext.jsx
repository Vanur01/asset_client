...
export const useRequestChecklist = () => {
  ...
  if (!context) {
    throw new Error("useRequestChecklist must be used within RequestChecklistProvider");
  }
  return context;
};
...
export const RequestChecklistProvider = ({ children }) => {
  ...
  const submitRequest = useCallback(async (requestData) => {
    ...
    setError(null);
    try {
      const response = await authRequest("POST", "/checklist/requests", requestData);
      if (response.success) {
  ...
  // Get all requests (Super Admin sees all, Admin sees their own)
  ...
  const getRequestById = useCallback(async (id) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || err.message || "Failed to fetch request";
      setError(errorMsg);
      return { success: false, error: errorMsg };
    } finally {
      setLoading(false);
  ...
  const reviewRequest = useCallback(async (id, action, reviewComments, createdChecklistId = null, rejectionReason = null) => {
    setLoading(true);
    setError(null);
    try {
      const body = { action, reviewComments };
      if (createdChecklistId) body.createdChecklistId = createdChecklistId;
      if (rejectionReason) body.rejectionReason = rejectionReason;
      
      const response = await authRequest("PATCH", `/checklist/requests/${id}/review`, body);
      if (response.success) {
